	...
	<body class="sidebar-mini wysihtml5-supported sidebar-collapse <?= $this->config->item('color')?>">
						...
						<div class="col-lg-6">
							<div class="panel panel-default">
								...
								<div class="form-group col-lg-4">
									...
									<input type="hidden" id="id_keluar" name="id_keluar" value='<?= $id_keluar?>' />
									...
									<input type="text" readonly id="tgl_exp" name="tgl_exp" class="form-control" placeholder="Tanggal Exp">
								</div>
								<div class="form-group col-lg-4">
									<label>Qty</label>
								...
								<div class="form-group col-lg-4">
									...
									<select class="form-control" id="allocation" name="allocation">
															<option value="0" >Manual Allocation</option>
															<option value="1" >FIFO</option>
															<option value="2" >FEFO</option>
														</select>
								</div>
		...
		<script type='text/javascript'>
			...
			var detailTable = $('#detailTable').DataTable({
				...
				ajax: 
				{
					url: "<?= base_url()?>keluar/ax_data_barang/",
					type: 'POST'
				},
				columns: 
				[
			...
			function Savedata(){
				...
				else if($('#qty').val() > $('#qtyd').val() && $('#allocation').val() == 0)
				{
				...
				else
				{
					...
					if($('#allocation').val() == 0)
					{
						var url = '<?=base_url()?>keluar/ax_set_details';
						var data = {
						id_keluar: <?= $id_keluar ?>,
						id_stok: $('#id_stok').val(),
						qty: $('#qty').val(),
						...
						var url = '<?=base_url()?>keluar/ax_set_allocation';
						var data = {
						id_keluar: <?= $id_keluar ?>,
			...
			function DeleteData(id_keluar_detail)
			{
				alertify.confirm(
					'Confirmation', 
					'Are you sure you want to delete this data?', 
					function(){
						var url = '<?=base_url()?>keluar/ax_unset_detail';
						var data = {
			...
			function PilihData(id_stok){
					...
					var url = '<?=base_url()?>keluar/ax_get_barang_by_id/';
					var data = {
						id_stok: id_stok
					};
